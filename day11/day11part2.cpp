...
{   
    ...
    for(int i = 0; i<10000; i++){
        for(Monkey currentMonkey: monkeys){
            while(currentMonkey.sItems.size() > 0){
                unsigned long worryNew = currentMonkey.sItems[0];
    ...
    unsigned long long tot = winner * second;
    cout<<tot<<"\n";
    cout<<winner<<"   " <<(winner==52166)<<"\n";
    cout<<second<<"   "<<(second==52013);
