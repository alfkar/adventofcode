...
using namespace std;
#define NeededSpace 30000000 - 25874010
...
void bestDir(Node * current, int goal){
    if(current->mySize >=  goal && current->mySize <= DirValue){
            DirValue = current->mySize;
        }
    if(current -> children.size() == 0){
        return;
    }
...
{   
    ...
    //cout<<totalSum;
    bestDir(currentNode, NeededSpace);
    cout<<DirValue;
}